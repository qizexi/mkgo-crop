...
function initMkgoCrop(mkCrop) {
	//屏幕大小
	var screenW = getScreenAvailW();
	var screenH = getScreenAvailH();
	
	//背景画布
	var bCObj = createCasObj();
	//裁剪画布
	var cCObj = createCasObj();
	//裁剪图像操作按钮
	...
	function getCasCenter(casobj) {
		...
		return {'cx': width/2, 'cy': height/2, 'zleng': zleng};
	}
